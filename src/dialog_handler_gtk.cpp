...
GtkWindow* GetWindow(CefRefPtr<CefBrowser> browser) {
  REQUIRE_MAIN_THREAD();
  GtkWidget* root_window = get_main_window();
  if (root_window) {
    return GTK_WINDOW(root_window);
  }
  return NULL;
}
...
ClientDialogHandlerGtk::ClientDialogHandlerGtk(std::string js_dialog_title)
      : gtk_dialog_(NULL) {
  js_dialog_title_ = js_dialog_title;      
}
